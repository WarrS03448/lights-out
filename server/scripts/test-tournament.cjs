...
function receipt(id='m1',at=start+1000,delta=25){return {matchId:id,data_collected:true,winner:1,publicMatch:{id,started:start,ended:at,outcome:'played',size:10,players:ids.map((p,i)=>({player_id:p,game_steam_id:p,persona:'Player '+i,team:i<5?1:2}))},rows:ids.map((p,i)=>({steamId:p,won:i<5,rr:{delta:i===0?delta:-10}}))};}
...
test('only full completed ranked receipts within the interval qualify',()=>{
 ...
 for(const mutate of [r=>r.publicMatch.started--,r=>r.publicMatch.ended=end,r=>r.publicMatch.size=2,r=>r.publicMatch.outcome='voided',r=>r.voided=true,r=>delete r.data_collected,r=>r.publicMatch.players.pop(),r=>r.rows[0].rr.delta=null]){const r=receipt();mutate(r);assert.equal(api.matchEntry(r),null);}
});
...
test('receipt order does not affect results; ties use wins then first reaching final total',()=>{
 ...
 const tied=api.standings([reg(),{...reg(),player_id:ids[1],game_steam_id:ids[1]}],[api.matchEntry(r)],{...api.EVENT,minimum_matches:1});assert.equal(tied[0].rank,1);assert.equal(tied[1].rank,1);assert.equal(tied[0].tied,true);
});
