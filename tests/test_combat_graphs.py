...
class CombatGraphs(unittest.TestCase):
    ...
    def test_auth_transport_and_game_mode_have_no_collector_state(self):
        ...
        self.assertIn(['http_auth_token.Search String', 'http_auth_cap.SourceString'], graph['links'])
        self.assertEqual(nodes['http_auth_cap']['defaults']['Count'], '64')
        self.assertIn(['http_auth_cap.ReturnValue', 'http_auth_prefix.A'], graph['links'])
